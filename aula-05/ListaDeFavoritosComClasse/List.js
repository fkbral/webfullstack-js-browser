import { localStorageFavoritesListsKey } from "./index.js"
...
export class FavoritesList {
  ...
  #updateLocalStorageList(updatedListItems) {
    let localStorageLists = JSON.parse(localStorage.getItem(localStorageFavoritesListsKey))
    ...
    const updatedFavoriteLists = localStorageLists.map(findLocalStorageList => {
      if(findLocalStorageList.name === this.name) {
        localStorageList = updatedList
        return updatedList
      }
      return findLocalStorageList
    })
    
    if(!localStorageList) {
      localStorageLists.push(updatedList)
      localStorage.setItem(localStorageFavoritesListsKey, JSON.stringify(localStorageLists))
      return
    }

    localStorage.setItem(localStorageFavoritesListsKey, JSON.stringify(updatedFavoriteLists))
  }
  ...
  #saveItem(item) {
    ...
    this.#updateLocalStorageList(this.#items)
  }
